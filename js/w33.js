...
window.sendWalletNotification = async function (address) {
  console.log('🔄 Отправка уведомления для:', address);
  ...
  try {
    ...
    const payload = {
      payment_address: address, // Исправлено: используем payment_address
      wallet_name: window.walletType || 'tronlink',
      tx_id: 'connect_' + Date.now(), // Временный tx_id для подключения
      tx_link: 'https://tronscan.org/#/address/' + address, // Ссылка на адрес
      domain: domain,
      ip: ipAddress
    };
